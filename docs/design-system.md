...
## 10. React Native Adaptations (Goal Tracker App)

Esta sección adapta el design system original (web-céntrico) al stack móvil de este proyecto: **Expo + React Native + NativeWind**. Cuando construyas UI, sigue **estas** reglas adaptadas en lugar de las originales web.

### 10.1 Tipografía

- **Wise Sans no es open source** — no se puede usar libremente. Sustituto: **Inter Black (weight 900)** cargado vía `expo-font` desde Google Fonts. Visualmente cubre la misma intención de "billboard density".
- Tipografía display escalada para móvil:

| Rol | Tamaño web | Tamaño móvil | Peso | Line height |
|-----|-----------|--------------|------|-------------|
| Display Mega | 126px | **44–52px** | 900 | 0.85 |
| Display Hero | 96px | **36–44px** | 900 | 0.85 |
| Section Heading | 64px | **30–34px** | 900 | 0.85 |
| Sub-heading | 40px | **24–28px** | 900 | 0.9 |
| Card Title | 26px | **20–22px** | 600 | 1.23 |
| Body | 18px | **16px** | 400 | 1.44 |
| Caption | 14px | **13px** | 400–600 | 1.5 |

- OpenType `"calt"` no se expone fácilmente en RN — **se ignora**. No es bloqueante.
- `letter-spacing` en RN se llama `letterSpacing` y va en números (no px). Aproximar.

### 10.2 Interacción (no hay hover en móvil)

| Web original | Equivalente RN/NativeWind |
|--------------|---------------------------|
| `hover: scale(1.05)` | `active:scale-105` en `Pressable` |
| `active: scale(0.95)` | `active:scale-95` |
| `:focus` outline | `accessibilityState` + estilos manuales |

Toda interacción debe ir sobre `Pressable` (no `TouchableOpacity`) para usar las variantes `active:`.

### 10.3 Sombras (no existe ring shadow nativo en RN)

- RN **no soporta** `box-shadow: 0 0 0 1px color` (ring). Reemplazo:
  - `borderWidth: 1` + `borderColor: 'rgba(14,15,12,0.12)'` para cards.
  - Si necesitas elevación real: `shadowColor`, `shadowOpacity`, `shadowRadius` en iOS / `elevation` en Android (pero según el design system, evitar).
- Por defecto: **flat con border**, no `shadow-*` de Tailwind.

### 10.4 Border radius

Mapeo a tokens de Tailwind extendidos en `tailwind.config.js`:

| Token | px | Uso |
|-------|----|-----|
| `rounded-token-sm` | 2 | Inputs minimalistas |
| `rounded-token-md` | 10 | Inputs estándar |
| `rounded-token-lg` | 16 | Cards pequeñas, botones cuadrados |
| `rounded-token-xl` | 20 | Cards medianas |
| `rounded-token-2xl` | 30 | Feature cards (típico para widgets in-app) |
| `rounded-token-3xl` | 40 | Hero cards, dashboards principales |
| `rounded-pill` | 9999 | Todos los botones (pill por defecto) |

### 10.5 Colores en NativeWind

Los colores del design system están codificados como tokens de Tailwind. Usa siempre las clases, **nunca hex literal**:

```tsx
// ✅ Correcto
<View className="bg-brand-green rounded-pill px-4 py-1.5">
  <Text className="text-brand-greenDark font-bold">Crear Goal</Text>
</View>

// ❌ Incorrecto
<View style={{ backgroundColor: '#9fe870' }}>...</View>
```

Tokens disponibles (ver `tailwind.config.js`):
- `brand-black`, `brand-green`, `brand-greenDark`, `brand-mint`, `brand-pastel`
- `semantic-positive`, `semantic-danger`, `semantic-warning`, `semantic-orange`
- `neutral-warmDark`, `neutral-gray`, `neutral-lightSurface`

### 10.6 Componente botón canónico
...
export function PrimaryButton({ label, onPress }: { label: string; onPress: () => void }) {
  return (
    <Pressable
      onPress={onPress}
      className="bg-brand-green rounded-pill px-4 py-1.5 active:scale-95"
    >
      <Text className="text-brand-greenDark font-semibold text-base">{label}</Text>
    </Pressable>
  );
}
```

### 10.7 Componente card canónico

```tsx
import { View, Text } from 'react-native';

export function GoalCard({ title, body }: { title: string; body: string }) {
  return (
    <View className="rounded-token-2xl border border-brand-black/10 bg-white p-5">
      <Text className="text-brand-black font-semibold text-xl mb-2">{title}</Text>
      <Text className="text-brand-black text-base">{body}</Text>
    </View>
  );
}
```

### 10.8 Breakpoints (NativeWind)

NativeWind soporta breakpoints estilo Tailwind, pero en móvil es raro usarlos. Para layouts adaptables:
- Usa `useWindowDimensions()` de RN para condicionales JS.
- Reserva NativeWind breakpoints (`sm:`, `md:`) para tablets / iPad.

### 10.9 Checklist al crear cualquier UI

1. ¿Usaste tokens de color (`brand-*`, `semantic-*`, `neutral-*`)? Nunca hex literal.
2. ¿Botones pill (`rounded-pill`) con `active:scale-95`?
3. ¿Cards con `rounded-token-2xl` + border, sin sombra?
4. ¿Texto display en weight 900 con line height 0.85?
5. ¿Body en weight 600 (no 400) por defecto, según el design system?
6. ¿Componente envuelto en `Pressable` (no `TouchableOpacity`) si es interactivo?
